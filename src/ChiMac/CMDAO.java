...
public class CMDAO {
	...
	public void memberList(String cId) {
		String sql = "SELECT CMID,CMPW,CMNAME,TO_CHAR(CMBIR,'YYYY/MM/DD'),CMGEN,CMEMAIL,CMADR,CMPHONE,CMBALANCE FROM CHIMAC WHERE CMID=? ";
	...
	// 맥주 4개 선택 만원
			public void fourbeerchoice(String one, String two, String three, String four) {
				String sql = "INSERT INTO BASKET VALUES(?,?)";
				
				try {
					pstmt = conn.prepareStatement(sql);
					
					pstmt.setString(1 ,one + ", "+ two + ", " + three + ", " + four);
	...
	public void withdraw1(String cId, int pay) {
		String sql = "UPDATE CHIMAC SET CMBALANCE = CMBALANCE - ? WHERE CMID = ?";
	...
	public String basketList4() {
		...
		try {
			...
			while (rs.next()) {
				...
				System.out.println("결제금액은" + rs.getString(1)+ "원 입니다 .");
